...
loci_file = '/storage/nmmsv/expansion-experiments/multi_locus_'+ref+'.txt'
...
exp_name = 'ART_MultiLoc_'+str(number) + \
			'_cov'+str(coverage)+\
...
for lc in loc_info:
        ...
        for nc1 in copy_list1:
                for nc2 in copy_list2:
                        ...
                        subprocess.call(['python',repo_dir + '2.3_simulate_reads_ART.py', \
                                         '--exp-name', 	exp_name, \
                                         '--fasta-in',	in_path, \
                                         '--out-pref', 	out_pref, \
                                         '--coverage',	str(art_cov), \
                                         '--num-copy',	str(nc), \
        ...
        with open(algn_read_dir + 'bamlist.txt', 'w') as bamlist:
                for nc1 in copy_list1:
                        for nc2 in copy_list2:
                                ...
                                subprocess.call(['python',repo_dir + '3.3_align_read_ART.py', \
                                                 '--ref-genome', ref_genome, \
                                                 '--out-pref', 	out_pref, \
                                                 '--in-pref', 	in_pref, \
